...
# Traveling_with_kids
response = HTTP.get("https://www.triposo.com/api/20200405/location.json?child_tag_labels=character-Kid_friendly|amusementparks|sightseeing&order_by=score&count=50&fields=intro,id,tag_labels,country_id&account=DIZGDG1D&token=")
response.parse["results"].each do |result|
  location = Location.create(
    name: result["id"],
    intro: result["intro"],
